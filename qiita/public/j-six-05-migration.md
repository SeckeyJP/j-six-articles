...
tags:
  ...
  - ai-development
  - si
updated_at: '2026-03-29T16:44:48.339Z'
id: null
...
:::note
本記事はシリーズ「**J-SIX：Japanese SI Transformation**」の #5（最終回）です。[#0 概要編](/seckeyjp/articles/j-six-00-overview)で全体像を把握してからお読みください。
:::
...
```mermaid
flowchart TB
  S0["Stage 0<br>従来V字モデル<br>自律度 L0"]
  S1["Stage 1<br>V字+CC補助<br>3ヶ月 / L1-L2"]
  S2["Stage 2<br>CC駆動ハイブリッド<br>3-6ヶ月 / L2-L3"]
  S3["Stage 3<br>J-SIX 全面適用<br>6ヶ月〜 / L3-L4"]
  S0 -->|"CC導入<br>効果計測開始"| S1
  S1 -->|"20%以上<br>工数削減確認"| S2
  S2 -->|"40%以上<br>品質同等確認"| S3
```
...
※ 工数削減率は著者推定の目標値です。個別事例では40-80%の生産性向上が報告されていますが[^anthropic-teams]、組織全体への適用時は控えめに見積もっています。
...
※ 工数削減率は各種事例報告[^anthropic-teams][^datacamp]を基にした著者推定の目標値であり、大規模な実証データに基づくものではありません。
...
| **Step 2: テストで囲む** | 変更対象の周囲に**特性化テスト**を書く。「正しい振る舞い」ではなく「現在の振る舞い」を記録するテスト[^feathers] | L2 |
| **Step 3: 島を作る** | 新規コードは J-SIX（TDD）で書く。Strangler Fig パターン[^fowler-strangler]で旧実装を徐々に置き換え | L3 |
...
| セキュリティ懸念（コードの外部送信） | Anthropic API の利用規約を確認。Enterprise プラン + VPC 環境の検討[^anthropic-enterprise] |
