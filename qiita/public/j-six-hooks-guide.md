...
:::note
**2026-09-20 追記**: Stop hook は、進捗のないまま8回連続でブロックすると CC に解除されます。また、ターンのたびに長いテストを待たされると、Hook そのものを無効にする運用に傾きがちです。J-SIX v2.1 では、Stop hook ではテストを実行せず既存の結果ファイルを読んで判定し、同じ判定を CI でも実行する二重構成にしました。詳しくは [4層品質ゲートの記事](https://zenn.dev/seckeyjp/articles/j-six-quality-gates) をご覧ください。
:::
...
Hook を Plugin として配布する場合は、さらに注意が要ります。prompt 型 Hook の書き方や実行タイミングの誤りで Hook が機能しなくなった実例は、[Plugin 実動検証の記事](https://zenn.dev/seckeyjp/articles/j-six-plugin-field-test)にまとめました。
