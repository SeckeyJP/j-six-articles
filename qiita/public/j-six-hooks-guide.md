...
:::note
本記事はシリーズ「**J-SIX：Japanese SI Transformation**」の番外編です。シリーズ全体の概要は [#0 概要編](https://qiita.com/SeckeyJP/items/e4726bbbbf4d7949ab0f)、TDD のアンチパターンは [TDD × AI の10のアンチパターン](https://qiita.com/SeckeyJP/items/245988ce4fca5b2ba74a) をご覧ください。
:::

## はじめに

J-SIX シリーズや [TDD アンチパターン記事](https://qiita.com/SeckeyJP/items/245988ce4fca5b2ba74a)で、「Hook で対策」「Hook でブロック」と繰り返し述べてきました。しかし Hook の具体的な実装——どのファイルに何を書いて、スクリプトはどう構成するのか——は一度も示していませんでした。
...
[TDD アンチパターン](https://qiita.com/SeckeyJP/items/245988ce4fca5b2ba74a)の「①実装ファースト偏向（Vibe TDD）」への対策です。CC はテストを後回しにして実装ファーストに自然回帰しやすいため[^hooks-guide]、**実装ファイルを編集する前に「テストが失敗していること」を機械的に確認** します。
...
[TDD アンチパターン](https://qiita.com/SeckeyJP/items/245988ce4fca5b2ba74a)の「③テスト改変（Gaming the Green）」への対策です。CC は実装を修正するよりテストのアサーションを変更する方が「効率的」と判断することがあります。**Green Phase 中にテストファイルを編集しようとしたらブロック** します。
...
[TDD アンチパターン](https://qiita.com/SeckeyJP/items/245988ce4fca5b2ba74a)の「⑨根本原因なきリトライ（Infinite Green Thrashing）」への対策です。CC はテスト失敗に対して局所的な修正を繰り返し、根本原因を分析しないことがあります。**3回連続でテストが失敗したら CC を停止させ、人間に判断を委ねます**。
...
:::note
**2026-09-20 追記**: Stop hook は、進捗のないまま8回連続でブロックすると CC に解除されます。また、ターンのたびに長いテストを待たされると、Hook そのものを無効にする運用に傾きがちです。J-SIX v2.1 では、Stop hook ではテストを実行せず既存の結果ファイルを読んで判定し、同じ判定を CI でも実行する二重構成にしました。詳しくは [4層品質ゲートの記事](https://qiita.com/SeckeyJP/items/735cf0618ae9e98c4c9e) をご覧ください。
:::
...
[Agent Teams](https://qiita.com/SeckeyJP/items/2639ae22b85e04a98c9c) と Hooks を組み合わせることで、マルチエージェント環境でも品質ゲートを自動化できます[^hooks-ref]。
...
Teammate がタスク完了を報告するたびにテストが自動実行され、失敗していれば exit code 2 でブロックされます。[Agent Teams 記事](https://qiita.com/SeckeyJP/items/2639ae22b85e04a98c9c)で紹介した Writer/Reviewer パターンと組み合わせれば、品質ゲートの自動化がより強固になります。
...
Hook を Plugin として配布する場合は、さらに注意が要ります。prompt 型 Hook の書き方や実行タイミングの誤りで Hook が機能しなくなった実例は、[Plugin 実動検証の記事](https://qiita.com/SeckeyJP/items/37448b1981004ceb5bb0)にまとめました。
