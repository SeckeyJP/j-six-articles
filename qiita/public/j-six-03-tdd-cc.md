...
tags:
  ...
  - SI
private: false
updated_at: '2026-09-20T01:01:50+09:00'
...
:::note
本記事はシリーズ「**J-SIX：Japanese SI Transformation**」の #3 です。シリーズ全体の概要は [#0 概要編](https://qiita.com/SeckeyJP/items/e4726bbbbf4d7949ab0f) をご覧ください。
:::

:::note
**2026-09-20 更新**: J-SIX v2.1 で自律度 L4 の条件を「自動テスト通過」から「4層品質ゲート（G1〜G4）の全層通過」に改めました。本記事の自律度モデル・TDD サイクルの図・並列実行・段階的移行の該当箇所を更新しています。
:::
...
| **L4: CC完全自律** | 全て自律実行 | 介入しない | **4層品質ゲート（G1〜G4）の全層通過**。自動テストだけでは L4 にしない |

### L4 の条件を「自動テスト通過」にしない理由

初版では L4 を「自動テスト + 自動品質チェックのみ」としていました。しかし、エージェントにとって唯一の監督面がテストスイートであるとき、テストスイートは検証手段ではなく**最適化の対象**になります。「テストは通るが意図を満たさない実装」が生まれるのです（reward hacking）。

SpecBench は、コーディングエージェントが見えているテストを通しながら、実利用を模した隠しテスト（hold-out）では落ちる現象を測定しました。両者の差はコード規模が大きくなるほど広がり、テスト入力を丸暗記する実装まで観測されています[^specbench]。「カバレッジを上げれば防げる」という素朴な対策では足りません。

そこで J-SIX は L4 の条件を、**性質の異なる4つの監督面をすべて通過すること**に改めました。

| 層 | 名称 | 内容 | 判定主体 |
|---|---|---|---|
| G1 | 決定論的検証 | build / lint / SAST / 依存脆弱性 / 変更ファイルがタスクの許可範囲内か | スクリプト |
| G2 | テスト品質検証 | テスト通過 / カバレッジ / mutation score / テストを弱める変更の検出 / **エージェントが書いていない hold-out 受入テスト**の通過 | スクリプト |
| G3 | 意図・スコープ判定 | 別コンテキストの judge が「正確性・要件未充足・スコープ逸脱」だけを判定 | LLM |
| G4 | 証跡パッケージ | G1〜G3 の結果を人間レビュー・顧客納品向けに構造化 | スクリプト＋LLM（要約のみ） |

順序は固定で、前段を通過したものだけが後段に進みます。4層の詳細は [J-SIX.md の Phase 4](https://github.com/SeckeyJP/j-six/blob/main/docs/J-SIX.md) で定義しています。G2 の mutation score については、カバレッジ 99% のコードで実測した記録を[番外編](https://qiita.com/SeckeyJP/items/b136d6d0bb2d830fb508)にまとめています。4層それぞれの設計の理由と、実際に動かして見つかった問題は[4層品質ゲートの記事](https://qiita.com/SeckeyJP/items/735cf0618ae9e98c4c9e)で解説しています。
...
| **Phase 4: TDD実装** | **L3-L4** | **TDD ＋ 4層品質ゲートがガードレール** |
...
コードを生成した CC とは別の CC インスタンスがレビューします。自分が書いたコードへのバイアスがないため、客観的なレビューが可能です[^anthropic-bp]。v2.1 ではこのレビューを G3（意図・スコープ判定）に統合しました。判定をスタイルの指摘に広げず、正確性・要件未充足・スコープ逸脱の3点に絞っています。
...
flowchart TD
    ...
    I --> J{"テスト通過<br>再確認?"}
    J -- Yes --> K["G1 決定論的検証<br>→ G2 テスト品質検証<br>→ G3 意図・スコープ判定"]
    J -- No --> I
    K --> L{"全層通過?"}
    L -- Yes --> M["G4 証跡パッケージ生成<br>タスク完了"]
    L -- No --> E
```
...
:::note
CLAUDE.md に TDD の手順を書いておけば、毎回この指示を繰り返す必要はありません。詳しくは [#4 CLAUDE.md 実践ガイド](https://qiita.com/SeckeyJP/items/d267f1c523eb95b2c9b5) で解説します。
:::
...
| パターン | 推奨自律度 | 理由 |
|---|---|---|
| 独立タスクの並列（依存関係なし） | L4 | 相互影響がないため完全自律可能（G1〜G4 の全層通過が条件） |
...
Hooks は CC のネイティブ機能であり、外部ツールへの依存なしにガードレールを構築できます[^anthropic-autonomous]。ただし Stop hook は、進捗のないまま8回連続でブロックすると解除されます[^anthropic-bp]。Hook は「絶対に止まる壁」ではないため、J-SIX では同じ判定を CI（外側ループ）でも実行し、Hook が外れても止まる二重構成にしています。
...
独立性の高いタスク（依存関係なし、影響範囲小）から完全自律に移行します。前提は、4層品質ゲート（G1〜G4）と CI による二重化が整っていることです。人間は証跡パッケージの確認と、異常検知時の介入に集中します。
