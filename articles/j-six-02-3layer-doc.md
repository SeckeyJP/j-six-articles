...
この作り方は、コードにある構造情報と、コードだけでは分からない判断理由を組み合わせます。設計書とコードの乖離を減らすには、対象版を固定し、生成後も内容を照合する必要があります。

## コードから復元できる情報/できない情報

設計書を「逆生成」する――つまり実装済みコードから設計書を作るアプローチは、同じ版から機械抽出した範囲の転記漏れを減らせます。LLM が記述する部分や組立・変換した表示まで、自動的に一致するわけではありません。富士通は2025年にAIによる設計書リバースエンジニアリングサービスを発表し、手動比50%の効率化を報告しています[^fujitsu]。
...
|  | コードに存在する | コードに存在しない |
|---|---|---|
| **構造的（静的情報）** | **抽出可能** クラス/関数構造、DBスキーマ、APIエンドポイント、依存関係、データ型/制約（対象版と抽出器の対応範囲による） | **部分復元可能** システム全体の配置構成、外部IFの相手先仕様、非機能要件の数値目標 |
| **意図的（暗黙知）** | **推測可能** 処理の振る舞い（テストから）、エラーハンドリング方針、命名規則 | **復元不可能** 設計判断の理由、却下した代替案、業務上の背景知識、将来の拡張計画 |

各象限を具体的に見てみましょう。

**構造として抽出できる情報**は、逆生成が最も得意とする領域です。クラス構造、DDL/マイグレーション、OpenAPI/ルーティングは、対象版と解析器の対応範囲を明示して抽出・照合できます。
...
|---|---|---|---|
| 構造的情報（What/How） | ○ 設計書に記載 | ○ 対応する範囲を機械抽出 | ○ 抽出後に照合 |
...
| 設計書とコードの整合性 | △ 乖離が生じ得る | ○ 対象版・抽出範囲を照合 | ○ 記述・組立も確認 |
...
3層戦略では、Spec で業務背景を、ADR で設計判断と代替案を、逆生成設計書で構造情報を記録します。整合性は対象版・抽出範囲・LLM 記述・組立結果を確認して初めて評価できます。逆生成された設計書は「実装がどうなっているか」の記録であり、「作るべきものか」の独立した検証ではありません。
...
4. **設計書とコードの乖離を減らす** — 対象版と抽出範囲を固定し、生成後に照合する
5. **顧客対応は段階的に進める** — 短期は Design Spec + プロトタイプ、中期は ADR 併用、長期は業界変革

設計書は「なくなる」のではなく、作り方を変えられます。乖離を減らし、設計判断を残すには、生成後の確認と Spec・ADR の継続的な更新が必要です。
