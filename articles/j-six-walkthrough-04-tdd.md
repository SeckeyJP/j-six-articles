...
```mermaid
flowchart TB
    H["Hold-out<br>実装担当と分けて作る受入テスト"] --> R["Red<br>テストが落ちることを確認"]
    R --> G["Green<br>最小実装"]
    G --> RF["Refactor"]
    RF --> G1{"G1 決定論的検証"}
    G1 -->|"不合格"| FIX["修正"]
...
## 1. まず、実装担当と分けて受入テストを書く

TDD というと「テストを書いてから実装する」ですが、J-SIX ではその前にもう1段あります。**hold-out 受入テスト**です。

これは Spec の受入条件だけを見て別の担当が書き、実装担当へ共有しないテストです。J-SIX の Plugin では、実装担当のサブエージェント（green-agent）に Hook を仕込み、対象ツールからの `tests/acceptance/` の直接読み取りと `tests/` への直接書き込みを拒否します。Bash・Grep など別経路を含む完全な不可視性や改変不能性は、この Hook だけでは保証できません。強制的な分離が必要なら、ファイル権限や実行環境を別にする必要があります。
...
当時の著者の作業記録では、**7回実行して PASS が3回、REJECT が4回**で、**コードへの指摘は0件**でした。著者の手元に保存されている元の `judge.json` は最終の1判定分で、非公開です。Hub の公開派生イベントは Git コミット・証跡・ゲート履歴などから組み立てたもので、最終 `judge.json` の判定内容を公開したものではありません。別に残る `gate-history.jsonl` の7件は鮮度チェックの失敗履歴であり、G3 の7回の内訳ではありません。各回の対象版・判定・理由を結ぶ全記録は残っていないため、この内訳は独立再集計できません。
...
**著者の当時の観察では、実装は1回で通り、設計書の追随に5回かかりました。**
...
hold-out は実装前に別の書き手が固定した監督面なので、この作業では勝手に変更しません。結果として、**設計書のほうを実装に合わせました**。テストが仕様を固定し、設計書がそれに従う、という関係がそのまま出た場面でした。
