...
まず数字です。テスト件数・カバレッジ・mutation score・トレーサビリティは当時の保存レポートに基づく測定値です。公開コードは固定版から検査を再実行できますが、当時の生レポート一式は GitHub に公開されていません。G3 の回数と内訳は著者の当時の観察であり、全判定履歴から独立再集計できません。

対象コードは J-SIX の[月次請求サンプルの固定コミット](https://github.com/SeckeyJP/j-six/tree/dd4ebbbc05b8323c8a5871fcb366c80f2aa02290/examples/monthly-billing)です。検証手順は同版の README、依存版は `requirements.txt`・`requirements-dev.txt`・`pyproject.toml` を参照してください。公開の [Hub 派生イベント](https://github.com/SeckeyJP/j-six-hub/tree/main/data/projects/monthly-billing) は件数・来歴の再集計用であり、元の非公開レポートや当時の全 G3 判定を含みません。
...
| 証跡から変換した納品物 | 5点 |
| G3（意図・スコープ判定）の実行回数 | **7回**（著者の当時の観察。全判定履歴は未保存） |
...
27点のうち19点はコードと受入テストを材料に逆生成します。機械抽出した部分も、対象版・抽出範囲・生成後の表示を照合して初めて整合を確認できます。LLM が記述した箇所や組立結果は意味と網羅性の確認が別途必要です。逆生成物だけでは「作るべきものを作ったか」は検証できません。

## 3. 面倒だったこと

### G3 が7回。しかもコードの指摘は0件

著者の当時の観察では、いちばん時間を取られたのがここです。7回のうち4回が REJECT で、**コードへの指摘はなく、設計書の追随に関する指摘でした**。各判定の原記録は全件保存されておらず、独立再集計はできません。
...
**著者の当時の観察では、実装は1回で通りました。** 逆生成の対象は19点あり、人間（や AI）は目につきやすい成果物だけを直して終わりにしがちです。それを判定者が毎回見つけてきます。

さらに、設計書を直すたびに差分が変わるので、**判定が無効になって再判定**になります。修正 → 再判定 → 新しい指摘 → 修正、という繰り返しでした。

J-SIX 自身が「G3 の却下率と却下理由を計測し、効かなくなったら外す」と定めています。当時の著者の記録では却下理由が**設計書の追随漏れ**だったので、「G3 を外す」のではなく「**設計書の追随を機械化する**」ほうが筋だと考えています。

### 判定基準を絞る必要があった

著者の当時の記録では6回目で、判定の指示を変えました。「件数・版数・項番のずれなど、読み手が要件を誤解しない軽微な差異は報告しない」と明記したのです。その結果 PASS になりました。
...
19点は実装などを材料に逆生成するため、手書き量を減らせると考えています。ただし作成・照合に要した時間は測っていません。人手で保つ5点と、生成した章を含む成果物どうしの整合（件数・版数・相互参照）も確認が必要です。当時の7回という著者観察は、この追随作業の難しさを示す一例です。
...
6回にわたり、1つの要件を通す様子をお見せしました。コード・Spec・設計書は GitHub の固定版で参照でき、公開部分の検査は手元で再実行できます。Hub の派生イベントも公開しています。一方、元のセッション記録・生レポート・過去 G3 全判定は公開しておらず、当時の数値や経過のすべてを第三者が再現できるわけではありません。
