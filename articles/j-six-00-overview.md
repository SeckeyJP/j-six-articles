...
emoji: "🔄"
type: "tech"
topics: ["claudecode", "ai", "開発プロセス", "SI"]
published: false
---
...
- **品質の最終責任は人間にある** — CC生成コードには人間の約1.7倍のイシューが含まれるとの調査報告あり[^coderabbit]
...
| Sonnet 4.5 コード編集エラー率 | 0%（内部ベンチマーク） | Anthropic 公式[^sonnet45] |
...
BMAD Method、Superpowers、GitHub Spec Kit、cc-sdd など複数のフレームワークが登場し、CGI やAnthropicを含む大手企業も採用を始めています[^cgi-sdd]。

共通する原則は4つあります。**Spec First**（仕様が Source of Truth）、**Phase Gate**（フェーズ完了時に人間が承認）、**TDD**（テスト駆動開発）、**並列エージェント**（Writer/Reviewer 分離）です。

:::details シリーズ #1 で詳しく解説します
...
Phase 1: 要求の合意（Spec 策定）            🚪 顧客承認
Phase 2: 技術設計（Design Spec + プロトタイプ）🚪 設計レビュー
Phase 3: タスク分解                          🚪 タスク承認
  ─── ここから CC 自律実行圏 ───
Phase 4: TDD 実装  ★生産性の源泉            🚪 自動テスト+CC相互レビュー
Phase 5: 品質検証                            🚪 品質基準達成判定
Phase 6: ドキュメント生成（3層戦略）          🚪 納品物レビュー
```
...
CC-Full プロセス（Phase 0-6）を全面適用。設計書は逆生成のみ。Agent Teams + git worktree で並列実行。

**目標**: 実装工数 **60-70%** 削減（著者推定）

### 投資対効果

5人チームの場合の試算です（CC Max 5x $100/月/人、人件費80万円/人月の仮定）。
...
|---|---|---|
| **#0** | **本記事（概要編）** | ✅ |
| #1 | V字モデルの前提崩壊と SDD の台頭 | 近日公開 |
| #2 | 3層ドキュメント戦略 — 設計書は「逆生成」の時代へ | 近日公開 |
| #3 | TDD × Claude Code — 自律実行で生産性を最大化する | 近日公開 |
| #4 | CLAUDE.md 実践ガイド — AI開発の「プロジェクト憲法」を書く | 近日公開 |
| #5 | V字モデルからの段階的移行 — 既存案件を止めずに J-SIX へ | 近日公開 |
