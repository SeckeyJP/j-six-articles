...
| 詳細設計書 + WBS | Phase 3: タスク分解 | 受入条件をテスト条件へ落とし込み、曖昧さを減らす |
| 実装 → 単体テスト | Phase 4: TDD 実装 | テストが先。CC が自律実行 |
| 結合テスト・総合テスト | Phase 5: 品質検証 | 自動テスト + 人間サンプリング |
| 各工程で設計書作成 | Phase 6: ドキュメント逆生成 | コードから生成し、対象版・生成範囲・記述を照合 |
...
- **SDD**: 設計意図（Spec + ADR）を先に書く → CC が TDD で実装 → コードから設計書を逆生成 → 対象版と抽出範囲を固定し、記述・組立結果も照合する
